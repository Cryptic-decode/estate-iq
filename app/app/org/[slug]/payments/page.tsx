...
import { PaymentsManager } from '@/components/app/payments/payments-manager'
...
export default async function PaymentsPage({
  ...
  searchParams: Promise<{ rentPeriodId?: string }>
}) {
  const { slug } = await params
  const { rentPeriodId } = await searchParams
  ...
  const [paymentsRes, rentPeriodsRes, rentConfigsRes, occupanciesRes, unitsRes, tenantsRes, buildingsRes, orgRes] =
    await Promise.all([
      listPayments(slug),
  ...
  return (
    <AppLayout orgSlug={slug} orgName={orgName} currentPath="payments" userRole={membership.role}>
      <PaymentsManager
        orgSlug={slug}
        orgName={orgName}
        currency={currency}
        ...
        preselectedRentPeriodId={rentPeriodId}
      />
    </AppLayout>
  )
}
