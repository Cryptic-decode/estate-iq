...
import { DelinquencyAgingReportView } from '@/components/app/reports/delinquency-aging-report'
import { getDelinquencyAging } from '@/app/actions/reports'
...
export default async function DelinquencyAgingPage({
  ...
  const reportRes = await getDelinquencyAging(slug)

  return (
    <AppLayout orgSlug={slug} orgName={orgName} currentPath="reports/delinquency-aging" userRole={membership.role}>
      <DelinquencyAgingReportView
        orgSlug={slug}
        orgName={orgName}
        currency={currency}
